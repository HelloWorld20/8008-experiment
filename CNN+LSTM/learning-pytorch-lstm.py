...
INPUT_DIR_PATH = '/Users/leon.w/workspace/cityu/8008/dataset/'
...
def read_data():
    ...
    '''
    sell_prices_df = pd.read_csv(INPUT_DIR_PATH + 'sell_prices.csv', nrows=100)
    ...
    sales_train_validation_df = pd.read_csv(INPUT_DIR_PATH + 'sales_train_validation.csv', nrows=100)
    print('Sales train validation has {} rows and {} columns'.format(sales_train_validation_df.shape[0], sales_train_validation_df.shape[1]))

    submission_df = pd.read_csv(INPUT_DIR_PATH + 'sample_submission.csv', nrows=100)
...
# 将数据转换为 numpy 数组，准备进行归一化
data = np.array(y)
# 初始化 MinMaxScaler，将数据缩放到 [-1, 1] 之间，这对 LSTM 训练非常重要，有助于加速模型收敛并减少梯度问题
...
def sliding_windows(data, seq_length):
    '''
    创建时间序列的滑动窗口数据集。
    输入原始数据和序列长度，返回用于模型训练的特征窗口 x 和对应的目标值 y。
    '''
    x = []
    y = []

    for i in range(len(data)-seq_length-1):
        # 遍历数据，按步长为 1 截取长度为 seq_length 的序列作为输入特征窗口
        _x = data[i:(i+seq_length)]
        # 紧跟在特征窗口后的下一个时间步作为需要预测的目标值
...
# 将 numpy 数组转换为 PyTorch 可用的 Tensor。
# 注：Variable 在新版 PyTorch 中已被弃用并合并进 Tensor，此处保留原有写法用于兼容
...
class LSTM(nn.Module):
    '''
    自定义的单层 LSTM 神经网络模型类。
    包含 LSTM 层和一个全连接输出层，主要用于时间序列预测任务。
    '''
    ...
    def forward(self, x):
        ...
        前向传播函数。
        根据输入序列 x 计算模型的输出预测值。
        参数 x 的形状应为: (batch_size, seq_length, input_size)
        '''
        # 初始化隐藏状态 h_0 和细胞状态 c_0，形状皆为 (num_layers, batch_size, hidden_size)
        ...
        # 传播输入通过 LSTM 层
        # ula 是包含所有时间步的输出的张量，h_out 包含每个隐层最后时间步的状态
        ula, (h_out, _) = self.lstm(x, (h_0, c_0))
        
        # 将 LSTM 最后的隐层状态展平为 (batch_size, hidden_size)，以便传入全连接层
        h_out = h_out.view(-1, self.hidden_size)
        
        # 全连接层将高维特征映射为预测目标，并应用 Dropout 防止过拟合
        out = self.fc(h_out)
...
class RMSELoss(nn.Module):
    '''
    自定义的均方根误差 (RMSE) 损失函数类。
    计算预测值和真实值之间的 MSE，然后对其开方。
    '''
    def __init__(self):
        super().__init__()
        self.mse = nn.MSELoss()
...
for epoch in progress_bar(range(num_epochs)): 
    # 切换为训练模式，启用 Dropout 等机制
    lstm.train()
    outputs = lstm(trainX.to(device))
    
    # 在每次反向传播前，必须将优化器中累积的梯度清零
    optimizer.zero_grad()
    
    # obtain the loss function
    # 计算当前批次预测值与真实值之间的均方误差损失
    loss = criterion(outputs, trainY.to(device))
    
    # 反向传播，计算损失相对于所有模型可学习参数的梯度
    loss.backward()
    ...
    #Evaluate on test     
    # 切换为评估模式，关闭 Dropout，保证验证时的前向传播结果确定
    lstm.eval()
    valid = lstm(testX.to(device))
    vall_loss = criterion(valid, testY.to(device))
    
    # 学习率调度器根据验证集上的损失决定是否需要降低学习率（如遇到性能瓶颈）
...
class LSTM2(nn.Module):
    '''
    自定义的双层 (多层) LSTM 神经网络模型类。
    包含两层 LSTM 和一个全连接输出层，用于捕获更复杂的时间序列模式。
    '''
    ...
    def forward(self, x):
        '''
        前向传播函数。
        处理输入特征 x，经过 LSTM 层提取隐藏状态，最后通过全连接层输出预测值。
        '''
