...
在训练过程中（特别是使用 wandb 时），需要密切关注以下核心指标的走势，它们能够反映整个端到端系统是否正常工作：

### 1. `mean_abs_grad` (代理梯度的绝对值均值 - 判断收敛的黄金指标)

- **含义**：代理模型（Surrogate Model）计算出的伪梯度 `surrogate_grad` 的绝对值平均大小。它的物理意义是：“如果预测量增加 1 个单位，业务成本会变化多少”。
- **目标**：**趋近于 0**。
  - 在 Predict-then-Optimize 框架下，由于存在固定的运费和持仓费，业务成本（Loss）**不可能降到 0**。
  - 因此，判断模型是否真正收敛的标准是：**梯度是否趋近于 0**。当 `mean_abs_grad` 稳定在一个极小值时，说明当前的预测量已经是最优的（稍微调大或调小都会导致成本上升），即模型已经到达了业务成本曲面的“极小值点”，完成了真正的收敛。
  - 如果该值一直很大，说明模型仍在剧烈调整预测值；如果该值突然变成 0 且 Loss 居高不下，说明陷入了“无论怎么预测求解器都不进货”的平坦区（通常是因为缺货惩罚 `penalty_coef` 太低）。

### 2. `mean_true_cost` (真实的业务成本均值)
